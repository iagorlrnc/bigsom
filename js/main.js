...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const animationObservers = new IntersectionObserver((entries) => {
    ...
    const animatedElements = document.querySelectorAll('.slide-up, .fade-in, .scale-in, .slide-right, .slide-left');
    animatedElements.forEach(el => animationObservers.observe(el));

    // Dynamic Gallery Generation
    const galleryContainer = document.getElementById('gallery-container');
    const galleryItems = [
        { img: 'public/assets/gallery-1.jpg', title: 'Porta Malas Personalizado' },
        { img: 'public/assets/gallery-2.jpg', title: 'Central Multimídia' },
        { img: 'public/assets/gallery-3.jpg', title: 'Projeto Alta Fidelidade' },
        { img: 'public/assets/gallery-4.jpg', title: 'Iluminação Interna LED' },
        { img: 'public/assets/gallery-5.jpg', title: 'Aplicação Insulfilm Nano' },
        { img: 'public/assets/gallery-6.jpg', title: 'Som para Competição' }
    ];
...
function initCanvasAnimation() {
    ...
    for (let i = 0; i < frameCount; i++) {
        ...
        imgD.src = `public/assets/${prefixDesktop}${i}.jpg`;
        imgD.onload = () => { imagesLoadedDesktop++; };
        imgD.onerror = () => { imgD.error = true; };
        imagesDesktop.push(imgD);

        const imgM = new Image();
        imgM.src = `public/assetsmobile/${prefixMobile}${i}.jpg`;
        imgM.onload = () => { imagesLoadedMobile++; };
        imgM.onerror = () => { imgM.error = true; };
    ...
    function renderLoop() {
        ...
        if (validImages.length > 5) { // If sequence works
